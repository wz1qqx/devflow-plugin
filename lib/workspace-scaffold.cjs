...
function workspaceReadme(name) {
  return [
    ...
    '',
    'The skeleton avoids obsolete `.dev.yaml` and project-specific `build.sh` assumptions. Reintroduce them as explicit recipes only when they are still useful.',
    '',
  ].join('\n');
}
...
function scaffoldWorkspace(options = {}) {
  ...
  if (options.cleanObsolete === true) {
    const obsoleteKnowledge = path.join(root, '.devteam', 'knowledge');
    if (fs.existsSync(obsoleteKnowledge)) {
      fs.rmSync(obsoleteKnowledge, { recursive: true, force: true });
      cleaned.push({ path: obsoleteKnowledge, action: 'removed' });
    }
  }
...
function handleWorkspaceScaffold(subcommand, args) {
  ...
  if (!subcommand || subcommand === 'scaffold') {
    output(scaffoldWorkspace({
      ...
      cleanObsolete: parsed['clean-obsolete'] === true,
    }));
    return;
  }

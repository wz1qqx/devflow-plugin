...
const { error, findWorkspaceRoot, expandHome } = require('./core.cjs');
const yaml = require('./yaml.cjs');

const LITE_DIR = '.devteam';
const LITE_CONFIG = 'config.yaml';

function findLiteRoot(startDir) {
  let dir = startDir || process.cwd();
  while (dir !== path.dirname(dir)) {
    if (fs.existsSync(path.join(dir, LITE_DIR, LITE_CONFIG))) return dir;
    dir = path.dirname(dir);
  }
  return null;
}

function resolveLiteRoot(rootArg) {
  if (rootArg) {
    const absolute = path.resolve(expandHome(rootArg));
    if (fs.existsSync(path.join(absolute, LITE_DIR, LITE_CONFIG))) return absolute;
    error(`.devteam/config.yaml not found under '${absolute}'`);
  }

  const liteRoot = findLiteRoot();
  if (liteRoot) return liteRoot;

  const legacyRoot = findWorkspaceRoot();
  if (legacyRoot) {
    error(
      'Found legacy workspace.yaml but no .devteam/config.yaml. ' +
      'Run `node lib/devteam.cjs lite migrate --from <legacy-workspace> --to <new-workspace>` for the lighter workflow.'
    );
  }
...
function configPath(root) {
  return path.join(root, LITE_DIR, LITE_CONFIG);
}
...
function normalizeLiteConfig(raw, root, cfgPath) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
...
function loadLiteConfig(rootArg) {
  const root = resolveLiteRoot(rootArg);
  ...
  return normalizeLiteConfig(parsed, root, cfgPath);
}

function ensureLiteDirs(root) {
  fs.mkdirSync(path.join(root, LITE_DIR), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'state'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'profiles'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'flows'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'recipes'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'wiki'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'skills'), { recursive: true });
  fs.mkdirSync(path.join(root, LITE_DIR, 'runs'), { recursive: true });
}

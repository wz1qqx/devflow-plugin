...
function loadConfig(workspaceRoot) {
  ...
  // Split format (workspace.yaml + per-feature config.yaml)
  const wsPath = path.join(root, 'workspace.yaml');
  if (fs.existsSync(wsPath)) return loadSplitConfig(root, wsPath);

  // Legacy format (.dev.yaml)
  const legacyPath = path.join(root, '.dev.yaml');
  if (fs.existsSync(legacyPath)) return loadLegacyConfig(root, legacyPath);

  error('workspace.yaml not found. Run /devteam init workspace to initialize.');
}
...
function loadLegacyConfig(root, legacyPath) {
  const yamlContent = fs.readFileSync(legacyPath, 'utf8');
  if (!yamlContent.trim()) error('.dev.yaml is empty. Run /devteam:init workspace to initialize it.');
  try {
    const raw = yaml.parse(yamlContent);
    if (!raw || typeof raw !== 'object') {
      error('.dev.yaml is empty or invalid. Run /devteam:init workspace to initialize it.');
    }
    const config = { _root: root, _path: legacyPath, _format: 'legacy', ...raw };
    if (config.schema_version !== 2) {
      error('Unsupported schema_version. Only v2 is supported.');
    }
    return config;
  } catch (err) {
    if (err.name === 'DevflowError') throw err;

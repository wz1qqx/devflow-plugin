...
/**
 * Append a build history entry to features.<name>.build_history in .dev.yaml.
 * Uses raw line manipulation (no YAML serialiser) to preserve file formatting.
 */
function appendBuildHistory(config, featureName, entry) {
  ...
  if (historyLine === -1) {
    ...
    // Find end of existing build_history list
    insertAt = featureEnd;
    for (let i = historyLine + 1; i < featureEnd; i++) {
      const line = lines[i];
      if (line.trim() === '') continue;
      const indent = (line.match(/^(\s*)/) || ['', ''])[1].length;
      if (indent <= fieldIndent) { insertAt = i; break; }
    }
...
module.exports = {
  ...
  switchActiveFeature, deleteFeature,
};

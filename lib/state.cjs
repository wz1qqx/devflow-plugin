  ...
  const formatYamlValue = (value) => {
    ...
    if (typeof value === 'object') {
      const entries = Object.entries(value).map(([key, val]) => {
        ...
        return `${normalizedKey}: ${formatYamlValue(val)}`;
      });
      return `{${entries.join(', ')}}`;
    }
...
function normalizeSourceRefList(sourceRefs) {
  ...
  for (const ref of sourceRefs) {
    ...
    if (typeof ref === 'object') {
      normalized.push({
        repo: ref.repo || null,
        start_head: ref.start_head || ref.sha || null,
        start_branch: ref.start_branch || ref.branch || null,
        dev_worktree: ref.dev_worktree || null,
      });
    }
  }
  return normalized;
...
function sourceRefToLabel(ref) {
  const repo = ref.repo || 'unknown-repo';
  const sha = ref.start_head || 'unknown';
  return `${repo}@${sha}`;
}

/**
 * Regenerate .dev/features/<name>/build-manifest.md from build_history.
...
function writeBuildManifest(root, featureName, entry) {
  ...
  const feature = requireFeature(config, featureName);
  const history = Array.isArray(feature.build_history) && feature.build_history.length > 0
    ? feature.build_history
    : [entry];

  const rows = history.map(item => {
    const parentImage = item.parent_image || item.base || null;
    const fallbackBaseImage = item.fallback_base_image || null;
    const resultingImage = item.resulting_image || null;
    const sourceRefs = normalizeSourceRefList(item.source_refs).map(sourceRefToLabel);
    return [
      `| ${normalizeCell(item.tag)}`,
      `${normalizeCell(item.date)}`,
      `${normalizeCell(parentImage)}`,
      `${normalizeCell(fallbackBaseImage)}`,
      `${normalizeCell(resultingImage)}`,
      `${normalizeCell(sourceRefs)}`,
      `${normalizeCell(item.changes)}`,
      `${normalizeCell(item.mode)}`,
      `${normalizeCell(item.cluster)} |`,
    ].join(' | ');
  });

  const sourceRefSections = [];
  for (const item of history) {
    const refs = normalizeSourceRefList(item.source_refs);
    if (refs.length === 0) continue;
    sourceRefSections.push(`### ${normalizeCell(item.tag || '-')}`);
    sourceRefSections.push('');
    sourceRefSections.push('| Repo | Branch | SHA | Dev Worktree |');
    sourceRefSections.push('|------|--------|-----|--------------|');
    for (const ref of refs) {
      sourceRefSections.push(
        `| ${normalizeCell(ref.repo)} | ${normalizeCell(ref.start_branch)} | ${normalizeCell(ref.start_head)} | ${normalizeCell(ref.dev_worktree)} |`
      );
    }
    sourceRefSections.push('');
  }
  ...
  const header = [
    ...
    '## Source Refs',
    '',
    ...(sourceRefSections.length > 0 ? sourceRefSections : ['_No source refs captured._', '']),
  ].join('\n');

  const rendered = header.endsWith('\n') ? header : `${header}\n`;

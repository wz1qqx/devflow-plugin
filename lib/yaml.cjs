...
/**
 * Lightweight YAML parser for .devteam/config.yaml and related small config files.
...
function stripComment(line) {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (ch === '"' && !inSingle && !isEscaped(line, i)) { inDouble = !inDouble; continue; }
...
function unescapeDoubleQuoted(s) {
  return s.replace(/\\([0abtnvfre "\\/N_LP])/g, (_match, ch) => {
    switch (ch) {
      case '0': return '\0';
      case 'a': return '\x07';
      case 'b': return '\b';
...
function splitTopLevel(s, delimiter) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (ch === "'" && !inDouble) { inSingle = !inSingle; continue; }
    if (ch === '"' && !inSingle && !isEscaped(s, i)) { inDouble = !inDouble; continue; }
...
function findColon(trimmed) {
  ...
  for (let i = 0; i < trimmed.length; i++) {
    const ch = trimmed[i];
    if (ch === "'" && !inDouble) { inSingle = !inSingle; continue; }
    if (ch === '"' && !inSingle && !isEscaped(trimmed, i)) { inDouble = !inDouble; continue; }
    if (inSingle || inDouble) continue;
    if (ch === '{') { braceDepth++; continue; }
    if (ch === '}') { braceDepth--; continue; }
    if (ch === '[') { bracketDepth++; continue; }

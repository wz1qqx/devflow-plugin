...
function isRunSnapshotCompleted(root, featureName) {
  const { loadStateMd } = require('./session.cjs');
  const state = loadStateMd(root, featureName);
  const featureStage = state && state.frontmatter ? state.frontmatter.feature_stage : null;
  return featureStage === 'completed';
}

function initRunState(root, featureName, options = {}) {
  const existing = readRunState(root, featureName);
  const staleCompleted = existing && !options.restart && isRunSnapshotCompleted(root, featureName);
  if (existing && !options.restart && !staleCompleted) {
  ...
  const action = options.restart
    ? (existing ? 'restart' : 'init')
    : 'init';
